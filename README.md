...
If deploying via TrueNAS SCALE's Apps &rarr; Discover Apps &rarr; **Install via YAML** instead of the CLI, copy [truenas-compose.example.yaml](truenas-compose.example.yaml) to `truenas-compose.yaml` (gitignored, since filling it in means putting real secrets inline) and fill in every `CHANGE_ME` (secrets, dataset path, `BETTER_AUTH_URL`) before pasting it into the UI. It's the same stack as [docker-compose.yml](docker-compose.yml) but with secrets inlined as `environment:` (the pasted-YAML flow has no `.env` file to read) and Postgres storage pointed at a real dataset path instead of a Docker-managed volume.
