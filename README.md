...
2. Run `docker compose up -d` using [docker-compose.yml](docker-compose.yml). This starts the app on port 3000 (with its own persistent volume for uploaded photos), a Postgres 16 instance with a persistent volume, and a [Watchtower](https://containrrr.dev/watchtower/) sidecar that polls GHCR every 5 minutes and auto-updates the app container when a new `latest` image is published (Postgres itself is pinned to the `16` tag and won't be auto-updated).
...
If deploying via TrueNAS SCALE's Apps &rarr; Discover Apps &rarr; **Install via YAML** instead of the CLI, copy [truenas-compose.example.yaml](truenas-compose.example.yaml) to `truenas-compose.yaml` (gitignored, since filling it in means putting real secrets inline) and fill in every `CHANGE_ME` (secrets, dataset paths, `BETTER_AUTH_URL`) before pasting it into the UI. It's the same stack as [docker-compose.yml](docker-compose.yml) but with secrets inlined as `environment:` (the pasted-YAML flow has no `.env` file to read) and Postgres/uploads storage pointed at real dataset paths instead of Docker-managed volumes.
